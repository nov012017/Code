...
# Run some basic diagnostic plots for the regression model lin_model
layout(matrix(c(1,2,3,4),2,2)) # Diagnostic plots - courtesy of https://www.statmethods.net/stats/regression.html 
plot(lin_model)

# Print a simple list of all variables, and count the number of missing values in each variable.
colSums(sapply(train, is.na))

#First, we have to convert all relevant variables to numeric variables - if we don't we won't be able to use them in our model.

glimpse(train)

#lin_model3 <- lm(price_new ~ ., data=train)
#summary(lin_model3)